...
export function AssetsPage(user) {
  return {
    ...
    setup() {
      ...
      return { inventory, selectedCategory, selectedStatus, selectedExpiry }
    },
    render(ctx) {
        ...
        child: {
          ...
          13: buildGrid({
            ...
            child: {
              ...
              8: buildDropdown({ placeholder: 'All Categories', items: [{ text: 'Laptop', value: 'laptop' }, { text: 'Monitor', value: 'monitor' }], value: ctx.selectedCategory, onUpdate: (v) => ctx.selectedCategory = v, showClearButton: true, width: '100%', height: '38px' }),
              10: buildDropdown({ placeholder: 'All Statuses', items: [{ text: 'Assigned', value: 'assigned' }, { text: 'Available', value: 'available' }], value: ctx.selectedStatus, onUpdate: (v) => ctx.selectedStatus = v, showClearButton: true, width: '100%', height: '38px' }),
              12: buildDropdown({ placeholder: 'Any Time', items: [{ text: 'Expired', value: 'expired' }], value: ctx.selectedExpiry, onUpdate: (v) => ctx.selectedExpiry = v, showClearButton: true, width: '100%', height: '38px' }),
              14: buildTextBadge('Clear', { variant: 'link', color: 'primary', size: 'sm', onPressed: () => { ctx.selectedCategory = null; ctx.selectedStatus = null; ctx.selectedExpiry = null }, style: { marginTop: '7px', fontWeight: '700' } }),
            },
          }),
          19: ctx.inventory.length > 0 ? buildTable({
            columns: [
